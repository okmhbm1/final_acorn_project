...
import com.bob.please.board.free.dao.BoardFreeCommentDao;
import com.bob.please.board.free.dao.BoardFreeDao;
...
public class BoardFreeServiceImpl implements BoardFreeService{
	...
	@Autowired
	private  BoardFreeCommentDao BoardFreeCommentDao;
	//한 페이지에 나타낼 row 의 갯수 
	...
	public void getList(HttpServletRequest request) {
		...
		//전체 row 의 갯수를 읽어온다.
		int totalRow=BoardFreeDao.getCount(dto);
		//전체 페이지의 갯수 구하기
		int totalPageCount=
		...
		List<BoardFreeDto> list=BoardFreeDao.getList(dto);
	...
	public void getDetail(HttpServletRequest request) {
		...
		BoardFreeDto resultDto=BoardFreeDao.getData(dto);
		//글 조회수 올리기
		BoardFreeDao.addViewCount(num);
		//request 에 글정보를 담고
		request.setAttribute("dto", resultDto);
		//댓글 목록을 얻어온다. (자세히 보기할 글번호가 댓글의 ref_group 번호)
		List<BoardFreeCommentDto> commentList=BoardFreeCommentDao.getList(num);
	...
	public void getUpdateData(ModelAndView mView, int num) {
		//글 수정 폼에 필요한 데이터를 얻어온다.
				BoardFreeDto dto=BoardFreeDao.getData(num);
	...
	public void saveComment(HttpServletRequest request) {
		//댓글 작성자
			...
			int seq=BoardFreeCommentDao.getSequence();
			//댓글 정보를 CafeCommentDto 객체에 담는다.
			BoardFreeCommentDto dto=new BoardFreeCommentDto();
			dto.setNum(seq);
			...
			//댓글 정보를 DB 에 저장한다.
			BoardFreeCommentDao.insert(dto);
		}

	@Override
	public void updateComment(BoardFreeCommentDto dto) {
		BoardFreeCommentDao.update(dto);
